...
- **audio/**：纯 MoonBit 音频脚步检测（20ms 帧能量、相对阈值、0.15s 去抖）
  ...
  中位数残差精修；再对配对残差做最小二乘拟合**漂移项**（双机录音时钟
  漂移 / PAL 提速 / 错采样率），累积不足一个容差（100ms）视为噪声不
  重采样。`align` 子命令提取原音轨、估计偏移+漂移、无漂移走采样级精确
  平移、有漂移走单次线性插值变速，重合成后保留原声音色。wav 解析兼容
...
- 多人（v0.4）：躯干列分组逐人跟踪，适用于**水平分开、不交叉遮挡**的
  ...
  轨迹缺帧，后者与 #6 同源）。交叉走位/互相遮挡仍不支持；align 仍按
  单主体设计，密集混合脚步会因邻近步混叠配错偏移。
